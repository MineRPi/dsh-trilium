...
export function triliumAppInfoTool(etapi) {
    return defineTool({
...
/** Full-text search. */
export function triliumSearchTool(etapi) {
    return defineTool({
        name: 'trilium_search',
...
export function triliumGetNoteTool(etapi) {
    return defineTool({
...
export function triliumListChildrenTool(etapi) {
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(value.children.length === 0
                ? '（' + value.noteId + ' 下没有子笔记）'
                : value.children.map(child => child.noteId + '  ' + child.title + (child.type !== 'text' ? '  [' + child.type + ']' : '') + (child.hasChildren ? '  ▸' : '') + (child.isProtected ? '  🔒' : '')).join('\n')),
        },
        async execute(args) {
...
/** Create a note. */
export function triliumCreateNoteTool(etapi, getConfig) {
    return defineTool({
...
export function triliumUpdateNoteTool(etapi) {
    return defineTool({
...
export function triliumDeleteNoteTool(etapi, getConfig) {
    return defineTool({
...
/** Undelete a note. */
export function triliumUndeleteNoteTool(etapi) {
    return defineTool({
...
export function triliumAttributeTool(etapi) {
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(value.attributes !== undefined
                ? '属性列表：\n' + renderAttributes(value.attributes)
...
export function triliumExportTool(etapi) {
    return defineTool({
        name: 'trilium_export',
...
/** Recent changes. */
export function triliumHistoryTool(etapi) {
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(value.changes.length === 0
                ? '（无变更记录）'
